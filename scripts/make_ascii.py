...
# defaults to the cropped source photo; raw photos work fine here since we
# tune contrast/gamma/white-floor below instead of relying on a pre-pass.
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "source-photo.png")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "komal-ascii.svg")
...
# working from a raw (non-background-removed) photo, so contrast is pushed
# harder and the white floor is lower to help a bright sky background wash
# out to blank while keeping facial detail legible.
CONTRAST = 1.35
BRIGHTNESS = 1.0
GAMMA = 1.05
SHARPEN = True
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">komal@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'komal@github:~$ whoami <tspan fill="{INK}">Komal</tspan></text>')
